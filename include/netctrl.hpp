...
private:
...
#ifdef _WIN32
    void runSilentAsync(const std::string& command) {
        std::string fullCmd = "cmd.exe /C start /B " + command + " >nul 2>&1";
        WinExec(fullCmd.c_str(), SW_HIDE);
    }
    ...
    bool applyWindows(double drop_percent) {
        if (drop_percent >= 100.0) {
            ...
            runSilentAsync("netsh advfirewall firewall add rule name=\"NetCtrl_OUT\" dir=out action=block protocol=any");
            runSilentAsync("netsh advfirewall firewall add rule name=\"NetCtrl_IN\" dir=in action=block protocol=any");
    ...
    bool disableWindows() {
        ...
        runSilentAsync("netsh advfirewall firewall delete rule name=\"NetCtrl_OUT\"");
        runSilentAsync("netsh advfirewall firewall delete rule name=\"NetCtrl_IN\"");
        is_active_ = false;
        current_lag_ms_ = 0;
